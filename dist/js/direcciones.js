direccionesModulo = (function () {
  ...
  function calcularYMostrarRutas () {
    ...
    var escalas = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
    var escalaIndex = 0;

    marcadorModulo.agregarMarcadorRuta(desde, escalas[escalaIndex++ % escalas.length], true);
    if (agregar !== ""){
      marcadorModulo.agregarMarcadorRuta(agregar, escalas[escalaIndex++ % escalas.length], true);
    }
    marcadorModulo.agregarMarcadorRuta(hasta, escalas[escalaIndex++ % escalas.length], true);

    servicioDirecciones.route(request, function(result, status) {
      if (status == 'OK') {
        mostradorDirecciones.setDirections(result);
        var panelDirecciones = document.getElementById('directions-panel');
        panelDirecciones.innerHTML = '';
      }
    });
